...
public class Garage {
    ...
    public boolean parkVehicle(Vehicle vehicle) throws ParkingException {
        ...
        if(vehicle.isExist(spots))
    		throw new ParkingException(ExceptionCode.VEHICLE_ALLREADY_EXIST.getMessage(), ExceptionCode.VEHICLE_ALLREADY_EXIST);
    ...
    private boolean parkStartingAtSpot(int number, Vehicle vhcl) {
        ...
        for (int i = number; i < number + vhcl.getSpotsNeeded(); i++) {
            ...
            logger.info("Allocating the spot.. " + i + " for vehicle :" + vhcl.vehiclePlate +" - "+ vhcl.getVehicleType());
        }
    ...
    public String status() {
        ...
        Map<String, List<ParkingSpot>> statusMap = Arrays.stream(spots).
                collect(Collectors.groupingBy(o -> o.getVehicle().getVehiclePlate()));
        return statusMap.keySet().stream()
                .map(key -> key + " " + statusMap.get(key).stream().map(ParkingSpot::getSpotNumber).collect(Collectors.toList()))
